          ...
          <div class="version-list">
            @for (version of versions(); track version.version_id) {
              <article class="card version-card">
                ...
                <div class="version-metrics"><span>Accuracy <strong>{{ version.accuracy | percent:'1.0-1' }}</strong></span><span>Precision <strong>{{ version.precision | percent:'1.0-1' }}</strong></span><span>Recall <strong>{{ version.recall | percent:'1.0-1' }}</strong></span><span>Rows <strong>{{ version.training_rows | number }} / {{ version.test_rows | number }}</strong></span></div>
              </article>
            }
          </div>
        </section>
