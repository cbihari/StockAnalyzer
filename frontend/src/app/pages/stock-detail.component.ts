    ...
    <main class="page">
      ...
      @if (prediction(); as result) {
        ...
        <div class="notice model-summary">
          ...
          @if (result.model_accuracy !== null) { <span>Model accuracy <app-info-tip text="Share of historical test predictions the model got right." /> {{ result.model_accuracy | percent:'1.0-1' }}</span> }
        ...
        @if (analysis(); as research) {
          <section class="research-summary-grid">
            <article class="prediction-card card" [class.down]="result.prediction === 'DOWN'"><span>NEXT TRADING-DAY ESTIMATE</span><div class="signal-line"><strong>{{ result.prediction }}</strong><b>{{ result.confidence }}% <app-info-tip text="How strongly the model favors its estimate based on current inputs." /></b></div><p>Model confidence, not probability of profit</p><div class="probability-track"><i [style.width.%]="result.probability_up * 100"></i></div><div class="probability-labels"><span>UP {{ result.probability_up | percent:'1.0-1' }}</span><span>DOWN {{ result.probability_down | percent:'1.0-1' }}</span></div></article>
            <article class="risk-card card" [class.high]="research.risk.level === 'HIGH'"><span>RISK LEVEL <app-info-tip text="Overall uncertainty based on volatility, signal agreement, and data quality." /></span><div class="risk-score"><strong>{{ research.risk.level }}</strong><b>{{ research.risk.score }}/100</b></div><div class="risk-track"><i [style.width.%]="research.risk.score"></i></div><p>{{ research.risk.summary }}</p></article>
            <article class="card model-reliability"><span>MODEL ACCURACY <app-info-tip text="Share of historical test predictions the model got right." /></span><strong>{{ result.model_accuracy === null ? 'N/A' : (result.model_accuracy | percent:'1.0-1') }}</strong><p>Accuracy across historical holdout cases, not this specific prediction.</p><small>{{ modelStatusLabel(result.model_status) }}</small></article>
          </section>
          <section class="market-context-grid">
            <article class="card level-card"><div class="section-title"><h2>Price structure</h2><span>{{ research.marketContext.lookbackSessions }} sessions</span></div><div class="level-values"><div><span>Support <app-info-tip text="A recent price area where declines have often slowed." /></span><strong>{{ research.quote.currency === 'INR' ? '₹' : '$' }}{{ research.marketContext.support | number:'1.2-2' }}</strong></div><div><span>Resistance <app-info-tip text="A recent price area where advances have often slowed." /></span><strong>{{ research.quote.currency === 'INR' ? '₹' : '$' }}{{ research.marketContext.resistance | number:'1.2-2' }}</strong></div></div><div class="range-track" aria-label="Current price position inside recent range"><i [style.left.%]="research.marketContext.rangePosition * 100"></i></div><div class="range-labels"><span>Recent low</span><b>{{ research.marketContext.rangePosition | percent:'1.0-0' }} through range</b><span>Recent high</span></div></article>
            <article class="card volatility-card"><div class="section-title"><h2>Volatility <app-info-tip text="How widely and quickly the stock price tends to move." /></h2><span>Annualized estimate</span></div><strong>{{ research.marketContext.annualizedVolatility | percent:'1.1-1' }}</strong><p>{{ volatilityLabel(research.marketContext.annualizedVolatility) }}</p><small>Average daily high-low range {{ research.marketContext.averageDailyRange | percent:'1.1-1' }}</small></article>
        ...
        @if (indicators(); as data) {
          <section class="card indicators"><div class="section-title"><h2>Technical indicators</h2><span>{{ data.latest.date }}</span></div>
            <div class="indicator-grid">
              <div><span>RSI 14 <app-info-tip text="Shows whether recent price moves may be overbought or oversold." /> <a routerLink="/learn/rsi">Learn</a></span><strong>{{ data.latest.RSI_14 | number:'1.2-2' }}</strong></div><div><span>SMA20 <app-info-tip text="Average closing price over the last 20 sessions." /> <a routerLink="/learn/sma">Learn</a></span><strong>{{ data.latest.SMA_20 | number:'1.2-2' }}</strong></div><div><span>SMA50 <app-info-tip text="Average closing price over the last 50 sessions." /> <a routerLink="/learn/sma">Learn</a></span><strong>{{ data.latest.SMA_50 | number:'1.2-2' }}</strong></div><div><span>EMA20 <app-info-tip text="Tracks the recent trend with extra weight on newer prices." /> <a routerLink="/learn/ema">Learn</a></span><strong>{{ data.latest.EMA_20 | number:'1.2-2' }}</strong></div><div><span>EMA50 <app-info-tip text="Tracks the medium-term trend with extra weight on newer prices." /> <a routerLink="/learn/ema">Learn</a></span><strong>{{ data.latest.EMA_50 | number:'1.2-2' }}</strong></div><div><span>MACD <app-info-tip text="Shows if price momentum is speeding up or slowing down." /> <a routerLink="/learn/macd">Learn</a></span><strong>{{ data.latest.MACD | number:'1.2-2' }}</strong></div><div><span>MACD signal <app-info-tip text="A smoothed MACD line used to spot momentum changes." /> <a routerLink="/learn/macd">Learn</a></span><strong>{{ data.latest.MACD_signal | number:'1.2-2' }}</strong></div><div><span>Volume change <a routerLink="/learn/volume">Learn</a></span><strong>{{ data.latest.volume_change | percent:'1.0-1' }}</strong></div>
            </div>
          </section>
        }
      }
